...
int main()
{
	...
	load_mnist_dataset("mnist_data_set", training_images, training_labels, testing_images, testing_labels);
	...
	*/
	...
	while (true) {
		...
		cv::Rect my(100, 100, 640 - 200, 480 - 200);
		frame = frame(my);

		cv::Mat mat;
		cv::resize(frame, mat, cv::Size(28, 28));
		mat = ~mat;
		mat.copyTo(frame(cv::Rect(0, 0, 28, 28)));
		cv_image<unsigned char> tmp(mat);
		matrix<unsigned char> img;
		assign_image(img, tmp);
		auto inputs = std::vector<matrix<unsigned char>>{ img };
		auto res = net(inputs);

		cvui::printf(frame, 20, 100, "%d", res[0]);
